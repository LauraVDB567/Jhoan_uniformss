...
const UserDetailScreen = (props) => {
  ...
  const getUserById = async (id) => {
    try {
      const res = await axios.get(URL +`/crudUsuarios/ConseguirUsuario/${id}`);
  ...
  const deleteUser = async () => {
    setLoading(true);
    try {
      await axios.delete(URL +`/crudUsuarios/EliminarUsuario/${user.id}`);
  ...
  const updateUser = async () => {
    try {
      await axios.put(URL+`/crudUsuarios/ActualizarUsuario/${user.id}`, {
        name: user.name,
        email: user.email,
        phone: user.phone,
      });
